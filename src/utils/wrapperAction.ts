...
export function createSafeAction<TInput, TOutput>(
  ...
  return async (
    ...
    if (!validatedFields.success) {
      const fieldErrors = validatedFields.error.flatten().fieldErrors as Partial<
    ...
    try {
      ...
      if (options?.revalidatePath) {
        ...
        revalidatePath(path);
        console.log(`[ACTION_REVALIDATE] [${actionCtx}] Шлях оновлено: ${path}`);
      }
    } catch (error: unknown) {
    ...
    if (options?.redirectTo) {
      ...
      console.log(`[ACTION_REDIRECT] [${actionCtx}] Виконується редирект на: ${target}`);
      redirect(target);
    }
